    ...
    if datasets_base.name.startswith('ds_'):
        ...
        if result:
            if json_output:
                ...
                print(f"\n  Validity: {'✓' if result.all_placements_valid else '✗'}")
                if not result.all_placements_valid:
                    for err in result.validity_errors:
                        print(f"    - {err}")
                print(f"  State match: {'✓' if result.system_state_matches else '✗'}")
                print(f"  Queue/Replica: {'✓' if result.queue_replica_consistent else '✗'}")
                print(f"  Placement found: {'YES' if result.placement_found_in_bruteforce else 'NO'}")
                print(f"  Matches BF optimal: {'YES' if result.matches_bf_optimal_placement else 'NO'}")
                if result.true_rtt is not None:
                    print(f"  True RTT (JSONL): {result.true_rtt:.6f}s")
                print(f"  Optimal RTT: {result.optimal_rtt:.6f}s")
                print(f"  Regret: {result.regret:.6f}s ({result.regret_percent:.2f}%)")
                print(f"  Rank: {result.rank}/{result.total_placements}")
        return
    ...
    for ds_dir in ds_dirs:
        ...
        if result:
            ...
            if not json_output:
                ...
                validity_status = "✓" if result.all_placements_valid else "✗"
                state_status = "✓" if result.system_state_matches else "✗"
                queue_status = "✓" if result.queue_replica_consistent else "✗"
                match_status = "MATCHED" if result.placement_found_in_bruteforce else "NOT FOUND"
                print(f"  Validity: {validity_status}  State: {state_status}  Queue: {queue_status}")
                print(f"  Placement: {match_status}")
                optimal_match = "YES ✓" if result.matches_bf_optimal_placement else "NO"
                print(f"  Matches BF optimal: {optimal_match}")
                if result.true_rtt is not None:
                    print(f"  True RTT (JSONL): {result.true_rtt:.6f}s")
                print(f"  Optimal RTT: {result.optimal_rtt:.6f}s")
                print(f"  Regret: {result.regret:.6f}s ({result.regret_percent:.2f}%)")
