...
from src.policy.gnn_cosim.model import KnativeSchedulerState, KnativeSystemState
...
class GNNCosimOrchestrator(Orchestrator):
    def __init__(
            self,
            env: Environment,
            data: SimulationData,
        ...
        logging.info("[GNN Cosim Orchestrator] Initialization starting")
        print("[GNN Cosim Orchestrator] Initialization starting")
    ...
    def initialize_state(self) -> KnativeSystemState:
        ...
        if self.initial_replicas:
            logging.info(f"GNNCosimOrchestrator: Using {len(self.initial_replicas)} pre-seeded replica sets")
            print(f"\n=== GNN Cosim: Using {len(self.initial_replicas)} pre-seeded replica sets ===")
        ...
        try:
            if hasattr(self, 'models') and self.models and hasattr(self.scheduler, 'set_models'):
                logging.info("[GNN Cosim Orchestrator] Passing models to scheduler...")
                self.scheduler.set_models(self.models)
                print("[GNN Cosim Orchestrator] Models passed to scheduler")
                logging.info("[GNN Cosim Orchestrator] Models passed to scheduler successfully")
            elif not self.models:
                logging.warning("[GNN Cosim Orchestrator] No models provided - scheduler will use fallback")
                print("[GNN Cosim Orchestrator] WARNING: No models provided - scheduler will use fallback")
            else:
                logging.warning(f"[GNN Cosim Orchestrator] Models check failed: hasattr={hasattr(self, 'models')}, models={self.models}, has_set_models={hasattr(self.scheduler, 'set_models')}")
        except Exception as e:
            logging.error(f"[GNN Cosim Orchestrator] Error passing models to scheduler: {e}")
            print(f"[GNN Cosim Orchestrator] ERROR: Failed to pass models to scheduler: {e}")
            import traceback
            traceback.print_exc()
            # Continue anyway - scheduler should handle missing models gracefully
        
        logging.info("[GNN Cosim Orchestrator] State initialization complete")
        return system_state

    def initializer_process(self) -> Generator:
        """Override to add error handling for state initialization."""
        try:
            logging.info("[GNN Cosim Orchestrator] Starting initializer process...")
            print("[GNN Cosim Orchestrator] Starting initializer process...")
            ...
            logging.info("[GNN Cosim Orchestrator] System state put in mutex")
            ...
            try:
                ...
                logging.warning(f"[GNN Cosim Orchestrator] Error registering warmup tasks: {e}")

            # Begin orchestration
            logging.info("[GNN Cosim Orchestrator] Starting gateway, monitor, autoscaler, and scheduler processes...")
            ...
            logging.info("[GNN Cosim Orchestrator] All processes started successfully")
            print("[GNN Cosim Orchestrator] All processes started successfully")
            
        except Exception as e:
            logging.error(f"[GNN Cosim Orchestrator] CRITICAL ERROR in initializer_process: {e}")
            print(f"[GNN Cosim Orchestrator] CRITICAL ERROR in initializer_process: {e}")
            import traceback
            traceback.print_exc()
    ...
    def monitor_process(self):
        logging.info(f"[ {self.env.now} ] GNN Cosim Orchestrator Monitor started")
