...
#   - space_with_network.json: Infrastructure configuration (disabled)
#   - workload.json: Workload trace with task types and client assignments
#   - run.log: Execution log (disabled)
...
COSIM_LOG_DIR="${BASE}/scripts_cosim/logs"
mkdir -p "${OUT_BASE}" "${RESULTS_DIR}" "${BASE}/logs" "${WORKLOAD_TEMPLATES_DIR}" "${COSIM_LOG_DIR}"
...
# TASK_TYPE_RATIOS=("50-50" "60-40")  # dnn1%-dnn2% ratios
...
# Backup original config + restore and cleanup on exit
  ...
  for replica_cfg in "${REPLICA_CFGS[@]}"; do
    ...
    for seed in "${SEEDS[@]}"; do
      for qdist in "${QUEUE_DISTS[@]}"; do
        ...
        # Generate deterministic infrastructure (once per dataset)
        ...
        if [ ! -f "${INFRA_FILE}" ]; then
          ...
          echo "[${DID}] Using existing infrastructure file"
        fi

        # Run executor (sequential). Let brute-force run fully (no cap)
        # Use temporary log file until we know if it succeeds
        TMP_LOG="$(mktemp)"
        # Real-time log for this dataset under scripts_cosim/logs
        # COSIM_RUN_LOG="${COSIM_LOG_DIR}/${DID}.log"
        # Truncate any previous log for this dataset
        # : > "${COSIM_RUN_LOG}"
        start_time=$(date +%s)
        
        # Run with timeout to prevent infinite hangs (3600s = 1 hour per dataset)
        # 2>&1 | tee -a '${COSIM_RUN_LOG}' > '${TMP_LOG}'
        timeout 3600 bash -c "
          cd '${BASE}' && \
        ...
        if [ -f "${RESULTS_DIR}/best.json" ]; then
          ...
          # DIFF: Copy ALL result files (not just optimal) to results subfolder
          # Previously only copied placement summaries, now copy all simulation results
          # mkdir -p "${OUT_DIR}/results"
          # ls -1 "${RESULTS_DIR}"/simulation_*.json 2>/dev/null | while read -r RESULT; do
          #   cp "$RESULT" "${OUT_DIR}/results/" || true
          # done

          # Copy placement summaries file (JSONL format - all placements in one file)
          PLACEMENT_COUNT=0
          if [ -f "${RESULTS_DIR}/placements.jsonl" ]; then
            mkdir -p "${OUT_DIR}/placements"
            cp "${RESULTS_DIR}/placements.jsonl" "${OUT_DIR}/placements/placements.jsonl" || true
            # Count number of placements (one per line in JSONL)
            PLACEMENT_COUNT=$(wc -l < "${OUT_DIR}/placements/placements.jsonl" 2>/dev/null || echo "0")
            rm -f "${RESULTS_DIR}/placements.jsonl" || true
          fi
          ...
          # Log progress with time (including placement count)
          echo "${DID} SUCCESS $(date '+%Y-%m-%d %H:%M:%S') ${duration}s RTT=${OPTIMAL_RTT}s placements=${PLACEMENT_COUNT} qdist=${qname} seed=${seed}" >> "${PROGRESS_LOG}"
