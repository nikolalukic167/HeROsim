...
def build_placement_rtt_hash_table(base_dir: Path, n_jobs: int = -1) -> Dict[Tuple[str, Tuple[Tuple[int, int], ...]], float]:
    """Parallel build of placement RTT hash table from JSONL files."""
    
    def _parse_jsonl_file(jsonl_path: Path) -> List[Optional[Tuple[str, Tuple[Tuple[int, int], ...], float]]]:
        """Parse a JSONL file and return list of (dataset_id, combo, rtt) tuples."""
        results = []
        try:
            ...
            with open(jsonl_path, 'r') as f:
                for line in f:
                    ...
                    try:
                        ...
                        # Convert placement_plan to sorted tuple of (node, platform) pairs
                        # Sort by task ID (keys are strings like "0", "1", "2", etc.)
    ...
    all_jsonl_files = list(base_dir.glob("ds_*/placements/placements.jsonl"))
    
    print(f"Building placement RTT hash table from {len(all_jsonl_files)} JSONL files using n_jobs={n_jobs}...")
    start_time = time.perf_counter()
    
    # Parse JSONL files in parallel
    parsed_lists: List[List[Optional[Tuple[str, Tuple[Tuple[int, int], ...], float]]]] = Parallel(n_jobs=n_jobs, prefer="processes")(
        delayed(_parse_jsonl_file)(Path(p)) for p in all_jsonl_files
    )
    
    # Flatten the results
    parsed: List[Optional[Tuple[str, Tuple[Tuple[int, int], ...], float]]] = []
    for result_list in parsed_lists:
        parsed.extend(result_list)
...
def build_graph(df_nodes, df_tasks, df_platforms) -> Data:
    ...
    # Simple logging about replica availability
    num_replica_dnn1 = int(has_dnn1_arr.sum())
    num_replica_dnn2 = int(has_dnn2_arr.sum())
    print(f"[build_graph] Replica summary: dnn1={num_replica_dnn1}, dnn2={num_replica_dnn2}, total_platforms={len(df_platforms)}")
