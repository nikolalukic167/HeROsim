...
# Grid definition
# Note: Do not vary seeds; keep the config's seed. Limit connectivity ≤ 0.50.
PROBS=(0.10 0.20 0.30 0.40 0.50)

# OU parameter variations (5 * 2 = 10)
OU_MUS=(0.20 0.30 0.35 0.40 0.50)
OU_SIGMAS=(0.10 0.20)

# Replica config matrix: (per_client per_server client_percentage server_percentage) → 2
REPLICA_CFGS=(
  "0 1 0.2 0.5"
  "1 2 0.4 0.8"
)
  ...
  for mu in "${OU_MUS[@]}"; do
    for sg in "${OU_SIGMAS[@]}"; do
      for cfg in "${REPLICA_CFGS[@]}"; do
        ...
        echo "[${DID}] p=${p} ou.mu=${mu} ou.sigma=${sg} per_client=${pc} per_server=${ps} client_pct=${cpct} server_pct=${spct}"
        ...
        jq --argjson p "${p}" \
           ...
           '.network.topology.connection_probability=$p
            ...
            | .startup_simulation.runtime.ou.mu=$mu
            | .startup_simulation.runtime.ou.sigma=$sg' \
           "${CFG_PATH}" > "${TMP_CFG}"
